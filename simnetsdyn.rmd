...
res_N_dyn<-future_pmap_dfr(params_N_dyn,sim_par_dyn, .progress=TRUE)
col_list=c("random","additive","regenerated")
means_N_dyn<-res_N_dyn%>%group_by(time,N,p1,p2)%>%summarise(across(col_list,mean))
vars_N_dyn<-res_N_dyn%>%group_by(time,N,p1,p2)%>%summarise(across(col_list,var))
```
