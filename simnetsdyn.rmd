...
sim_dyn<-function(N=20,phiv=0.1,PrEP1=0.1,PrEP2=0.2, p1=0.2,p2=0.1,model=c("ER","BA","WS"),eprob=0.1,pow=1,nb=5,rprob=0.05, ReWire=F){
  ...
  prep<-cbind(hiv_given_prep_g,hiv_given_prep_h,hiv_given_prep_j,hiv_given_prep_k)
  names(prep)<-c("hiv_given_prep_g","hiv_given_prep_h","hiv_given_prep_j","hiv_given_prep_k")
  no_prep<-cbind(hiv_given_no_prep_g,hiv_given_no_prep_h,hiv_given_no_prep_j,hiv_given_no_prep_k)
  names(no_prep)<-c("hiv_given_no_prep_g","hiv_given_no_prep_h","hiv_given_no_prep_j","hiv_given_no_prep_k")
  ef<-prep+no_prep
  colnames(ef)<-c("control","random","additive","regenerated")
  #compute causal contrast estimates
cc<-as.data.frame(ef[,-1]-ef[,1])
  colnames(cc)<-c("random","additive","regenerated")
  #spread epidemic 
  ...
  g_list[[i]]<-g
  #update risks
  hiv_given_prep_g[i]<-ifelse(length(treat_g)!=0,(length(treat_inf_contact_g)*p2)/length(treat_g),0)
  hiv_given_no_prep_g[i]<-ifelse(length(no_treat_g)!=0,(length(no_treat_inf_contact_g)*p1)/length(no_treat_g),0)
#random allocation
  ...
  h_list[[i]]<-h
  #update risks
  hiv_given_prep_h[i]<-ifelse(length(treat_h)!=0,(length(treat_inf_contact_h)*p2)/length(treat_h),0)
  hiv_given_no_prep_h[i]<-ifelse(length(no_treat_h)!=0,(length(no_treat_inf_contact_h)*p1)/length(no_treat_h),0)
  #additive allocation
  ...
  j_list[[i]]<-j
  #update risks
  hiv_given_prep_j[i]<-ifelse(length(treat_j)!=0,(length(treat_inf_contact_j)*p2)/length(treat_j),0)
  hiv_given_no_prep_j[i]<-ifelse(length(no_treat_j)!=0,(length(no_treat_inf_contact_j)*p1)/length(no_treat_j),0)
#regenerated
  ...
  k_list[[i]]<-k
  #update risks
   hiv_given_prep_k[i]<-ifelse(length(treat_k)!=0,(length(treat_inf_contact_k)*p2)/length(treat_k),0)
   hiv_given_no_prep_k[i]<-ifelse(length(no_treat_k)!=0,(length(no_treat_inf_contact_k)*p1)/length(no_treat_k),0)
#update combined risks/effects of PrEP
   prep[i]<-cbind(hiv_given_prep_g[i],hiv_given_prep_h[i],hiv_given_prep_j[i],hiv_given_no_prep_k[i])
   no_prep[i]<-cbind(hiv_given_no_prep_g[i],hiv_given_no_prep_h[i],hiv_given_no_prep_j[i],hiv_given_no_prep_k[i])
   ef[i]<-prep[i]+no_prep[i]
