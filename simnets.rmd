...
p<-1 #preferential attachment power for BA model
...
# plots=T
scale="additive"
# params<-list(N=20,eprob=0.1,phiv=0.1,PrEP1=0.1,PrEP2=0.2, p1=0.2,p2=0.1, plots=F, scale="additive")
```

## Simulation Function

```{r simulation}
sim<-function(N=20,phiv=0.1,PrEP1=0.1,PrEP2=0.2, p1=0.2,p2=0.1, plots=F, scale=c("additive","multiplicative"),model=c("ER","BA"),eprob=0.1,p=1){
   #parameter check
scale<-match.arg(scale)
model<-match.arg(model)
# plot a random graph, 3 color options
#control scenario graph, 10% assignment prob
g<-if(model=="ER"){sample_gnp(N,eprob)} else if(model=="BA"){sample_pa(N,power=p,directed=FALSE)}
...
# plot a random graph, 3 color options
k <- if(model=="ER"){sample_gnp(N,eprob)} else if(model=="BA"){sample_pa(N,power=p,directed=FALSE)}
...
res<-cbind(N,eprob,p,phiv,PrEP1,PrEP2,p1,p2,model,as.data.frame(t(prep)),as.data.frame(t(no_prep)),cc, as.data.frame(t(stats_g)),as.data.frame(t(stats_k)))
names(res)<-c("N","eprob","p","phiv", "PrEP1","PrEP2","p1","p2","model",names(prep), names(no_prep),names(cc),names(stats_g),names(stats_k))
return(res)
}
```
...
sim_par<-function(N=20,phiv=0.1,PrEP1=0.1,PrEP2=0.2, p1=0.2,p2=0.1, scale="additive",model="ER",eprob=0.1,p=1,nsim=100){
res<-future_map_dfr(1:nsim,~sim(N=N,phiv=phiv,PrEP1=PrEP1,PrEP2=PrEP2,p1=p1,p2=p2,scale=scale,model=model,eprob=eprob,p=p))
res<-cbind(res,nsim)
return(res)}
```
...
```{r}
params_p<-expand.grid(N=50,phiv=0.1,PrEP1=0.2,PrEP2=0.4,p1=seq(0.1,1,0.1),p2=seq(0.1,1,0.1), model="BA",p=seq(0.1,1,0.1), nsim=200,stringsAsFactors = F)
