...
res5<-foreach(p1=l1,.final=function(p1)setNames(p1,names(l1))) %:% foreach(p2=l2,.final=function(p2)setNames(p2,names(l2))) %do% {sim_par(p1=p1,p2=p2,nsim=20)}
```


```{r}
# hists<-rapply(res5,hist,how="replace")
meds<-rapply(res5,median)
medians<-split(meds,f=names(meds))
avgs<-rapply(res5,mean)
means<-split(avgs,f=names(avgs))
```
