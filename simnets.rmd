...
PrEP2<-0.3 # PrEP assignment coverage in counterfactual treatment (a*)
...
vertex_attr(g) <- list(color =c(rep("red", ceiling(gorder(g)*phiv)), rep("blue",ceiling(gorder(g)*PrEP1)), rep("black", ceiling(gorder(g)*(1-(phiv+PrEP1))))))
...
h<-set_vertex_attr(g,"color",value=gtools::permute(c(rep("red", ceiling(gorder(g)*phiv)), rep("blue",ceiling(gorder(g)*PrEP2)), rep("black", ceiling(gorder(g)*(1-(phiv+PrEP2)))))))
...
j<-set_vertex_attr(g,"color",value=c(rep("red", ceiling(gorder(g)*phiv)), rep("blue",ceiling(gorder(g)*PrEP2)), rep("black", ceiling(gorder(g)*(1-(phiv+PrEP2))))))
...
vertex_attr(k) <- list(color =c(rep("red", ceiling(gorder(k)*phiv)), rep("blue",ceiling(gorder(k)*PrEP1)), rep("black", ceiling(gorder(k)*(1-(phiv+PrEP1))))))
...
sim_par<-function(N=20,eprob=0.1,phiv=0.1,PrEP1=0.1,PrEP2=0.2, p1=0.2,p2=0.1, nsim=100 ){
res<-future_map_dfr(1:nsim,~sim(N=N,eprob=eprob,phiv=phiv,PrEP1=PrEP1,PrEP2=PrEP2, p1=p1,p2=p2))
...
```{r}
params_N<-tidyr::expand_grid(N=c(20,200),phiv=phiv,PrEP1=PrEP1,PrEP2=PrEP2,p1=seq(0.1,1,0.1),p2=seq(0.1,1,0.1), nsim=nsim)
future::plan(
      list(
        future::tweak(
          future::multisession, 
...
```{r}
N_plot1<-ggplot(means_N,aes_string(x="p1",y="p2", fill=col_list[1]))+geom_tile()+viridis::scale_fill_viridis(discrete=F,limits=c(-1,1))+facet_grid(~N)
N_plot2<-ggplot(means_N,aes_string(x="p1",y="p2", fill=col_list[2]))+geom_tile()+viridis::scale_fill_viridis(discrete=F,limits=c(-1,1))+facet_grid(~N)
N_plot3<-ggplot(means_N,aes_string(x="p1",y="p2", fill=col_list[3]))+geom_tile()+viridis::scale_fill_viridis(discrete=F,limits=c(-1,1))+facet_grid(~N)
...
params_phiv<-expand.grid(N=20,phiv=seq(0.1,0.3,0.1),PrEP1=PrEP1,PrEP2=PrEP2,p1=seq(0.1,1,0.1),p2=seq(0.1,1,0.1),nsim=200)
future::plan(
      list(
        future::tweak(
          future::multisession, 
...
```{r}
phiv_ran1<-ggplot(means_phiv,aes_string(x="p1",y="p2",fill=col_list[1]))+geom_tile()+facet_grid(~phiv)
phiv_ran2<-ggplot(means_phiv,aes_string(x="p1",y="p2",fill=col_list[2]))+geom_tile()+facet_grid(~phiv)
phiv_regen<-ggplot(means_phiv,aes_string(x="p1",y="p2",fill=col_list[3]))+geom_tile()+facet_grid(~phiv)
...
```{r}
p1_ran1<-ggplot(means_p1,aes(x=p1,y=p2))+aes(fill=col_list[1]+geom_tile()
p1_ran1
```
