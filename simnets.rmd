...
# future::plan(
      ...
      #   )
      # )
system.time(res_samp<-future_pmap_dfr(params_samp,sim_par, .options = furrr_options(seed = TRUE,),.progress = TRUE))
...
```{r}
params_model<-expand.grid(N=50,phiv=0.1,model=c("ER","BA","WS"),PrEP1=PrEP1,PrEP2=PrEP2,p1=seq(0.1,1,0.1),p2=seq(0.1,1,0.1),nsim=nsim, stringsAsFactors = F)
plan("multisession",workers=availableCores()/4)
...
```{r}
params_model_large<-expand.grid(N=N,phiv=0.1,model=c("ER","BA","WS"),PrEP1=PrEP1,PrEP2=PrEP2,p1=seq(0.1,1,0.1),p2=seq(0.1,1,0.1),nsim=nsim, stringsAsFactors = F)
plan("multisession",workers=availableCores()/4)
res_model_large<-future_pmap_dfr(params_model_large,sim_par)
means_model_large<-res_model_large%>%group_by(across(all_of(c("model","p1","p2"))))%>%summarise(across(col_list,mean))
var_model_large<-res_model_large%>%group_by(across(all_of(c("model","p1","p2"))))%>%summarise(across(col_list,var))
save(res_model_large,means_model_large,var_model_large,file=paste("N=",N,"Generative Model Results.RData"))
```
...
```{r}
params_ba<-expand.grid(N=50,phiv=0.1,PrEP1=0.2,PrEP2=0.4,p1=seq(0.1,1,0.1),p2=seq(0.1,1,0.1), model="BA",pow=pow, nsim=nsim,stringsAsFactors = F)
res_ba<-future_pmap_dfr(params_ba,sim_par)
means_ba<-res_ba%>%group_by(across(all_of(c("p1","p2"))))%>%summarise(across(col_list,mean))
var_ba<-res_ba%>%group_by(across(all_of(c("p1","p2"))))%>%summarise(across(col_list,var))
save(res_ba, means_ba,var_ba,file="BA check Results.RData")
lims<-c(min(means_ba[,col_list[2]]),max(means_ba[,col_list[2]]))
ggplot(means_ba,aes_string(x="p1",y="p2",fill=col_list[2]))+geom_tile()+scale_fill_gradientn(limits=lims,colors=brewer.pal(6,name="RdYlBu"))+xlab("P(HIV|Contact and -PrEP)")+ylab("P(HIV|Contact and PrEP)")
```
...
```{r PrEP subset}
means_PrEP%>%filter(PrEP1%in%seq(0.1,0.8,0.1),PrEP2%in%seq(0.3,0.9,0.2))%>%ggplot(aes_string(x="p1",y="p2",fill=col_list[1]))+geom_tile()+facet_wrap(PrEP1~PrEP2,scales="free")+scale_fill_gradientn(limits=lims_means_PrEP, colors=brewer.pal(5,name="RdYlBu"))+xlab("P(HIV|Contact and -PrEP)")+ylab("P(HIV|Contact and PrEP)")+ggtitle("Contrast Estimate Mean in random PrEP Allocation by PrEP Coverage")+scale_x_discrete(guide = guide_axis(n.dodge=2))
means_PrEP%>%filter(PrEP1%in%seq(0.1,0.8,0.1),PrEP2%in%seq(0.3,0.9,0.2))%>%ggplot(aes_string(x="p1",y="p2",fill=col_list[2]))+geom_tile()+facet_wrap(PrEP1~PrEP2,scales="free")+scale_fill_gradientn(limits=lims_means_PrEP, colors=brewer.pal(5,name="RdYlBu"))+xlab("P(HIV|Contact and -PrEP)")+ylab("P(HIV|Contact and PrEP)")+ggtitle("Contrast Estimate Mean in additive PrEP Allocation by PrEP Coverage")+scale_x_discrete(guide = guide_axis(n.dodge=2))
...
```{r}
cbroot<-function(x){sign(x) * (abs(x)) ^ (1/3)}
root_means_model<-means_model%>%group_by(across(all_of(c("model","p1","p2"))))%>%summarise(across(col_list,cbroot))
```

```{r}
lims_root_means_model<-c(min(root_means_model[,col_list]),max(root_means_model[,col_list]))
root_model_plot_1<-ggplot(root_means_model,aes_string(x="p1",y="p2",fill=col_list[1]))+geom_tile()+scale_fill_gradientn(limits=lims_root_means_model,colors=brewer.pal(10,name="RdYlBu"))+facet_grid(~model)+xlab("P(HIV|Contact and -PrEP)")+ylab("P(HIV|Contact and PrEP)")
root_model_plot_2<-ggplot(root_means_model,aes_string(x="p1",y="p2",fill=col_list[2]))+geom_tile()+scale_fill_gradientn(limits=lims_root_means_model,colors=brewer.pal(10,name="RdYlBu"))+facet_grid(~model)+xlab("P(HIV|Contact and -PrEP)")+ylab("P(HIV|Contact and PrEP)")
root_model_plot_3<-ggplot(root_means_model,aes_string(x="p1",y="p2",fill=col_list[3]))+geom_tile()+scale_fill_gradientn(limits=lims_root_means_model,colors=brewer.pal(10,name="RdYlBu"))+facet_grid(~model)+xlab("P(HIV|Contact and -PrEP)")+ylab("P(HIV|Contact and PrEP)")
...
```{r}
log_means_model<-means_model%>%group_by(across(all_of(c("model","p1","p2"))))%>%mutate(across(col_list,log1p))
```


```{r}
lims_log_means_model<-log1p(c(min(means_model[,col_list]),max(means_model[,col_list])))
log_model_plot_1<-ggplot(means_model,aes_string(x="p1",y="p2",fill=col_list[1]))+geom_tile()+scale_fill_gradientn(limits=lims_log_means_model,colors=brewer.pal(10,name="RdYlBu"),trans="log1p")+facet_grid(~model)+xlab("P(HIV|Contact and -PrEP)")+ylab("P(HIV|Contact and PrEP)")
log_model_plot_2<-ggplot(means_model,aes_string(x="p1",y="p2",fill=col_list[2]))+geom_tile()+scale_fill_gradientn(limits=lims_log_means_model,colors=brewer.pal(10,name="RdYlBu"))+facet_grid(~model)+xlab("P(HIV|Contact and -PrEP)")+ylab("P(HIV|Contact and PrEP)")
log_model_plot_3<-ggplot(means_model,aes_string(x="p1",y="p2",fill=col_list[3]))+geom_tile()+scale_fill_gradientn(limits=lims_log_means_model,colors=brewer.pal(10,name="RdYlBu"))+facet_grid(~model)+xlab("P(HIV|Contact and -PrEP)")+ylab("P(HIV|Contact and PrEP)")
 ggsave(plot=log_model_plot_2/log_model_plot_1/log_model_plot_3+plot_annotation(title = "Log-transformed Contrast Estimate Mean by Network Generative Model and Allocation" ) & 
  theme(plot.title = element_text(hjust = 0.5)),
file="Generative Model Log Mean plots.png", path="Figures")
```
...
```{r}
lims_means_model_ER<-means_model%>%filter(model=="ER")%>%select(col_list)%>%slice(c(which.min(), which.max()))
ER_only_plot_1<-means_model%>%filter(model=="ER")%>%ggplot(aes_string(x="p1",y="p2",fill=col_list[1]))+geom_tile()+scale_fill_gradientn(colors=brewer.pal(10,name="RdYlBu"))+facet_grid(~model)+xlab("P(HIV|Contact and -PrEP)")+ylab("P(HIV|Contact and PrEP)")
ER_only_plot_2<-means_model%>%filter(model=="ER")%>%ggplot(aes_string(x="p1",y="p2",fill=col_list[2]))+geom_tile()+scale_fill_gradientn(colors=brewer.pal(10,name="RdYlBu"))+facet_grid(~model)+xlab("P(HIV|Contact and -PrEP)")+ylab("P(HIV|Contact and PrEP)")
ER_only_plot_3<-means_model%>%filter(model=="ER")%>%ggplot(aes_string(x="p1",y="p2",fill=col_list[3]))+geom_tile()+scale_fill_gradientn(colors=brewer.pal(10,name="RdYlBu"))+facet_grid(~model)+xlab("P(HIV|Contact and -PrEP)")+ylab("P(HIV|Contact and PrEP)")
...
```{r}
lims_means_model_large<-c(min(means_model_large[,col_list]),max(means_model_large[,col_list]))
model_large_plot_1<-ggplot(means_model_large,aes_string(x="p1",y="p2",fill=col_list[1]))+geom_tile()+scale_fill_gradientn(limits=lims_means_model_large,colors=brewer.pal(11,name="RdYlBu"),breaks=waiver())+facet_grid(~model)+xlab("P(HIV|Contact and -PrEP)")+ylab("P(HIV|Contact and PrEP)")
model_large_plot_2<-ggplot(means_model_large,aes_string(x="p1",y="p2",fill=col_list[2]))+geom_tile()+scale_fill_gradientn(limits=lims_means_model_large,colors=brewer.pal(11,name="RdYlBu"))+facet_grid(~model)+xlab("P(HIV|Contact and -PrEP)")+ylab("P(HIV|Contact and PrEP)")
model_large_plot_3<-ggplot(means_model_large,aes_string(x="p1",y="p2",fill=col_list[3]))+geom_tile()+scale_fill_gradientn(limits=lims_means_model_large,colors=brewer.pal(11,name="RdYlBu"))+facet_grid(~model)+xlab("P(HIV|Contact and -PrEP)")+ylab("P(HIV|Contact and PrEP)")
 ggsave(plot=model_large_plot_2/model_large_plot_1/model_large_plot_3+plot_annotation(title = "N=200 Contrast Estimate Mean by Network Generative Model and Allocation" ) & 
  theme(plot.title = element_text(hjust = 0.5)),
file=" N=200 Generative Model Mean plots.png", path="Figures")
```
...
samp_plot_4<-ggplot(var_samp,aes_string(x="p1",y="p2", fill=col_list[1]))+geom_tile()+facet_grid(~nsim)+scale_fill_gradientn(limits=lims_var_samp,colors=brewer.pal(6,name="RdYlBu"))+ylab("P(HIV|Contact and PrEP)")+xlab("P(HIV|Contact and -PrEP)")
...
```{r}
lims_var_model_large<-c(min(var_model_large[,col_list]),max(var_model_large[,col_list]))
model_largeplot_4<-ggplot(var_model_large,aes_string(x="p1",y="p2",fill=col_list[1]))+geom_tile()+scale_fill_gradientn(limits=lims_var_model_large,colors=brewer.pal(10,name="RdYlBu"))+facet_grid(~model)+xlab("P(HIV|Contact and -PrEP)")+ylab("P(HIV|Contact and PrEP)")
model_largeplot_5<-ggplot(var_model_large,aes_string(x="p1",y="p2",fill=col_list[2]))+geom_tile()+scale_fill_gradientn(limits=lims_var_model_large,colors=brewer.pal(10,name="RdYlBu"))+facet_grid(~model)+xlab("P(HIV|Contact and -PrEP)")+ylab("P(HIV|Contact and PrEP)")
model_largeplot_6<-ggplot(var_model_large,aes_string(x="p1",y="p2",fill=col_list[3]))+geom_tile()+scale_fill_gradientn(limits=lims_var_model_large,colors=brewer.pal(10,name="RdYlBu"))+facet_grid(~model)+xlab("P(HIV|Contact and -PrEP)")+ylab("P(HIV|Contact and PrEP)")
ggsave(model_largeplot_5/model_largeplot_4/model_largeplot_6+    # Create grid of plots with title
  plot_annotation(title = "N=200 Contrast Estimate Variance by Network Generative Model and Allocation") & 
  theme(plot.title = element_text(hjust = 0.5)), file="N=200 Generative Model Variance plots.png",path = "Figures/")
```
